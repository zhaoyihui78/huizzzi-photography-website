...
- **文章类型系统**：
  - `article`：常规深度长文，双栏排版，显示在 `/notes` 列表页。
...
### 14. 手记术语系统扩展与移动端适配（2026-05-17）
- **状态**: 已完成
- **Glossary 扩展**:
  - 为《为什么城市摄影要等那10分钟》补充 10+ 个摄影术语入口，包括 `蓝调时刻`、`焦段`、`曝光`、`色温`、`对焦`、`景深`、`ISO`、`快门速度`、`白平衡`、`机位`、`构图`、`光圈`、`后期` 等。
  - 新增多篇 `glossary` 词条 MDX 文件，继续沿用隐藏彩蛋模式，不出现在 `/notes` 主列表。
- **移动端阅读优化**:
  - `src/app/notes/page.tsx`：缩小手机端留白、优化卡片间距与封面比例、提高标题和摘要可读性。
  - `src/app/notes/[slug]/page.tsx`：优化手机端标题、元信息换行、正文节奏和封面比例，并在手机端补回 tags 信息。
  - `src/app/globals.css`：调整 `.prose-custom` 的标题层级、段落行高、图片与引用块间距。
  - `src/components/GlossaryLink.tsx`：Glossary 悬浮提示仅在桌面显示，避免遮挡手机阅读。
  - `src/components/NoteBackButton.tsx`：优化返回按钮在小屏幕上的间距与字距。
