...
**恢复计划**：
- [ ] 等待作者从相机存储卡、手机相册、iCloud、百度网盘等其他渠道找回原始 `.mp4` 备份。
- [ ] 找回后重新压缩至适合 Web 播放的码率（1080p / 4Mbps），单文件控制在 50MB 以内，直接随 Vercel 部署，不再依赖 GitHub Release。
- [ ] 如无法找回，则暂时隐藏四季巡礼页面的视频播放入口，仅保留封面图与文字介绍。
...
| 视频托管 | ⚠️ **资产丢失** | 5 部核心视频原始 `.mp4` 在 2025-05-13 丢失，四季巡礼页面视频播放功能暂不可用，待找回备份后恢复。 |
...
- **视频外链加载**：GitHub Release 直链在国内网络环境下可能偶发延迟，后续可考虑接入 Cloudflare R2 或国内对象存储作为备源。
...
- [ ] 评估将视频从 GitHub Release 迁移至 Cloudflare R2 / 国内 CDN，提升国内访问速度。