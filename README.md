...
- **即时显示**: 采用 Optimistic Update 策略，点击发布后新留言立即出现在墙上，无需等待 GitHub API 缓存刷新
- **发布动效**: 信封飞出动画 + 卡片弹簧掉落动画，形成完整的发布反馈链
- **新留言高亮**: 24 小时内发布的留言带有金色高亮标记
- **数据持久**: 表单草稿自动保存到 `localStorage`，误刷新不丢失
