...
export default function GlossaryLink({ term, slug, description }: GlossaryLinkProps) {
  return (
    <Link
      href={`/notes/${slug}`}
      className="relative inline-block font-normal text-[#111111] group align-baseline"
    >
      ...
      <span className="hidden md:block absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-max max-w-[240px] bg-[#111111] text-white text-[12px] font-light leading-relaxed px-4 py-3 rounded-sm opacity-0 translate-y-2 pointer-events-none transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0 z-50 shadow-xl">
